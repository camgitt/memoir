...
// ── 2026-09-04 audit: assistant prose never mints decisions; machinery
// turns never reach the handoff; lower-case captures are rejected ─────
{
  console.log(`\n${BOLD}${CYAN}capture sources (2026-09-04)${RESET}\n`);
  const { generateContextHandoff, stripAnsi, isMachineryMessage } = await import('./src/context/capture.js');
  const scratch3 = await fs.mkdtemp(path.join(os.tmpdir(), 'memoir-capture-test3-'));
  const f3 = path.join(scratch3, 'session.jsonl');
  const meta = (t) => JSON.stringify({ type: 'user', isMeta: true, message: { content: t } });
  const lines = [
    // assistant prose that USED to mint a decision (issue #7 self-reinforcement)
    asst("We decided to go with Redis for the cache layer."),
    asst("Let's use Postgres for the queue instead of RabbitMQ."),
    // the two real junk rows from the author's store, as the user typed them
    usr("ok so the name is settled then"),
    usr("rename the app from gymlogger to something else"),
    // Claude Code machinery delivered as user turns
    meta("<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>"),
    usr("<command-name>/model</command-name>\n<command-message>model</command-message>"),
    usr("<local-command-stdout>Set model to \x1b[2mFable 5.1\x1b[22m and saved</local-command-stdout>"),
    usr("<task-notification>agent finished</task-notification>"),
    // real messages (10 of them, so the handoff's last-8 window is testable)
    ...Array.from({ length: 10 }, (_, i) => usr(`real message number ${i + 1} about the \x1b[32mgreen\x1b[0m feature`)),
    // a real decision, from the user
    usr("We decided to go with Postgres for the database layer."),
  ];
  await fs.writeFile(f3, lines.join('\n') + '\n');
  const p3 = parseSession(f3);
  const vals = p3.decisions.map(d => `${d.type}:${String(d.value)}`.toLowerCase());
  assert(!vals.some(v => v.includes('redis')), 'assistant "we decided to go with Redis" does NOT mint a decision');
  assert(!vals.some(v => v.includes('postgres') && v.includes('queue')), 'assistant "let\'s use Postgres for the queue" does NOT mint a decision');
  assert(!vals.some(v => v.endsWith(':settled')), '"the name is settled" does not mint the value "settled"');
  assert(!vals.some(v => v.endsWith(':from')), '"rename the app from …" does not mint the value "from"');
  assert(vals.some(v => v.startsWith('design:postgres')), 'a real user decision still lands');
  const um = p3.userMessages;
  assert(!um.some(m => m.includes('local-command-caveat')), 'isMeta caveat turn is excluded from userMessages');
  assert(!um.some(m => m.startsWith('<command-name>')), '<command-name> turn is excluded');
  assert(!um.some(m => m.startsWith('<local-command-stdout>')), '<local-command-stdout> turn is excluded');
  assert(!um.some(m => m.startsWith('<task-notification>')), '<task-notification> turn is excluded');
  assert(um.some(m => m === 'real message number 1 about the green feature'), 'ANSI codes are stripped from real messages');
  assert(um.length === 13, `real messages are all kept (${um.length})`);
  ...
  assert(!md.includes('/model') && !md.includes('\x1b['), 'handoff contains no command blocks or ANSI');
  await fs.remove(scratch3);
}
